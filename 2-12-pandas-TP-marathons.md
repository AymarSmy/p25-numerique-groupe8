...
```{code-cell} ipython3
# votre code
df = pd.read_csv('data/marathon.txt', sep = '\t', header = None, names = names)
df.head(10)
```
...
```{code-cell} ipython3
# à vous
s1_paris_2000 = df[(df['city'] == 'PARIS') & (df['year'] >= 2000)]['year']
s1_paris_2000
```
...
```{code-cell} ipython3
by_city = df.groupby(by='city')
count_by_city = by_city.size()
count_by_city
```
...
```{code-cell} ipython3
by_city['seconds'].mean()
duree_moyenne = pd.to_timedelta(by_city['seconds'].mean(), unit='seconds')
duree_moyenne
```
...
```{code-cell} ipython3
duree_moyenne.sort_values(ascending=True)
```
...
```{code-cell} ipython3
df['seconds'] = pd.to_timedelta(df['seconds'], unit='seconds')
(df['seconds'] == df['duration']).sum() == len(df)
```
...
```{code-cell} ipython3
df['hour'] = df['seconds']//one_hour
df['minute'] = (df['seconds']%one_hour)//one_minute
df['second'] = ((df['seconds']%one_hour)%one_minute)//one_second
df
```
...
```{code-cell} ipython3
df.drop(columns=['hour','minute','second'])
```
...
```{code-cell} ipython3
df['hour'] = df['seconds'].dt.components.hours
df['minute'] = df['seconds'].dt.components.minutes
df['second'] = df['seconds'].dt.components.seconds
```
