...
```{code-cell} ipython3
df['Sex'].unique() #Quelles sont les valeurs uniques d'une data frame
```
...
```{code-cell} ipython3
df1 = (df['Age'] >= 20) & (df['Age'] <= 40)
df1.sum()
dftot = (df['Survived'] == 0) | (df['Survived'] == 1)
dftot.sum()

taux20 = df1.sum()/dftot.sum()
taux20
```
...
```{code-cell} ipython3
df['Pclass'].value_counts()
dfC = df[['Pclass', 'Sex', 'Survived']]

#for i in range (1,4):
    totH = [347]
    dfi_S_H = (dfC['Sex']=='male') & (dfC['Survived']==1) & (dfC['Pclass'] == i)
    dfi_S_F = (dfC['Sex']=='female') & (dfC['Survived']==1) & (dfC['Pclass'] == i)
#    taux_i_S_H = dfi_S_H.sum()/totH
#    taux_i_S_F = dfi_S_F.sum()/totF
