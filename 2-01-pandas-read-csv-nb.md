...
```{code-cell} ipython3
df1 = pd.read_csv('data/petit-titanic.csv')
df1
```
...
```{code-cell} ipython3
df = pd.read_csv('data/petit-titanic.csv', sep = ';', header = None, names = ['PassengerId', 'Survived', 'Pclass', 'Name', 'Sex','Age', 'SibSp', 'Parch', 'Ticket', 'Fare', 'Cabin', 'Embarked'])
df
```
...
```{code-cell} ipython3
df[(df['Age'] < 12) & (df['Genre'] == 'female') & (df['Pclass'] == 2)]
```
